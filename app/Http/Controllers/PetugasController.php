...
class PetugasController extends Controller
{
    public function index(){
        $kelolaPerangkat = DB::table('request_pemasangan')->get();
        // Contoh pengambilan jumlah "Belum Diproses" dari database
        $aktifCount = DB::table('request_pemasangan')->where('status', 'Aktif')->count();
        $nonAktifCount = DB::table('request_pemasangan')->where('status', 'Non Aktif')->count();
        return view('petugas.kelolaPerangkat', compact('kelolaPerangkat', 'aktifCount', 'nonAktifCount'));
    }

    public function detailPerangkat($id){
        $detailPerangkat = DB::table('request_pemasangan')->where('id', $id)->first();
    ...
    public function store_pemasangan(Request $request, $id)
    {
        $validatedData = $request->validate([
            ...
            'id_perangkat.required'=>'DEVICE UID wajib diisi',
    ...
    public function laporanPengaduan(){
        ...
        return view('petugas.laporanPengaduan', compact('laporanPengaduan', 'belumDiprosesPengaduanCount', 'sedangDiprosesPengaduanCount', 'sudahSelesaiPengaduanCount','user'));
    }

    public function detailPengaduan(Request $request, $id_pelanggan)
    {
        ...
        $detailPengaduan = DB::table('pengaduan')->where('id_pelanggan', $id_pelanggan)->first();
